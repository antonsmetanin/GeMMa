...
public class UICamera : MonoBehaviour
{
	...
	public class MouseOrTouch
	{
		...
		public UIReceiver current;		// The current game object under the touch or mouse
		public UIReceiver pressed;		// The last game object to receive OnPress
		public UIReceiver dragged;		// The last game object to receive OnDrag
	...
	static public UIReceiver genericEventHandler;

	/// <summary>
	/// If events don't get handled, they will be forwarded to this game object.
	/// </summary>

	static public UIReceiver fallThrough;
	...
	static public UIReceiver hoveredObject;

	/// <summary>
	/// Option to manually set the selected game object.
	/// </summary>

	static public UIReceiver selectedObject
	{
		get
		{
			return mSel;
		...
		set
		{
			if (mSel != value)
			{
				if (mSel != null)
				{
					UICamera uicam = FindCameraForLayer(mSel.gameObject.layer);
...
//						Notify(mSel, "OnSelect", true);
	...
	static public bool IsHighlighted (UIReceiver go)
	{
...
//				Notify(go, "OnHover", true);
	...
	void FixedUpdate ()
	{
		...
		{
			hoveredObject = Raycast(Input.mousePosition, ref lastHit) ? lastHit.collider.gameObject.GetComponent<UIReceiver>() : fallThrough;
...
//                Notify(mHover, "OnScroll", scroll);
	...
	public void ProcessTouches ()
	{
		for (int i = 0; i < Input.touchCount; ++i)
		{
			...
			hoveredObject = Raycast(currentTouch.pos, ref lastHit) ? lastHit.collider.gameObject.GetComponent<UIReceiver>() : fallThrough;
			if (hoveredObject == null) hoveredObject = genericEventHandler;
...
//			Notify(currentTouch.pressed, "OnPress", false);
            if (currentTouch.pressed != null) currentTouch.pressed.OnPress(false);
...
//			Notify(currentTouch.pressed, "OnPress", true);
            if (currentTouch.pressed != null) currentTouch.pressed.OnPress(true);
		...
		else
		{
			...
			if (!stickyPress && !unpressed && currentTouch.pressStarted && currentTouch.pressed != hoveredObject)
			{
				isDragging = true;
//				Notify(currentTouch.pressed, "OnPress", false);
                currentTouch.pressed.OnPress(false);
				currentTouch.pressed = hoveredObject;
//				Notify(currentTouch.pressed, "OnPress", true);
                currentTouch.pressed.OnPress(true);
				isDragging = false;
			}
...
//						Notify(currentTouch.dragged, "OnDrag", currentTouch.delta);
                        currentTouch.dragged.OnDrag(currentTouch.delta);
						isDragging = false;
...
//                    Notify(currentTouch.pressed, "OnHover", true);
                    currentTouch.pressed.OnHover(true);
...
//						Notify(currentTouch.pressed, "OnSelect", true);
                        currentTouch.pressed.OnSelect(true);
					}
					else
					{
						mSel = currentTouch.pressed;
...
//						Notify(currentTouch.pressed, "OnClick", null);
                        currentTouch.pressed.OnClick();

						if (currentTouch.clickTime + 0.35f > time)
						{
//							Notify(currentTouch.pressed, "OnDoubleClick", null);
                            currentTouch.pressed.OnDoubleClick();
						}
